...
class DecodingStrategy(metaclass=abc.ABCMeta):
    ...
    def step(
        ...
        num_starts: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> TensorDict:
        """Main decoding operation. This method should be called in a loop until all sequences are done.
        ...
        if step == 0:
            # 1. 准备 3D 张量
            # logprobs 是 (Batch*Start, N) -> 还原为 (Batch, Start, N)
            # 我们需要用这个 3D 张量来做最后的 gather，保证索引对齐
            logprobs_3d = rearrange(logprobs, "(s b) l -> b s l", s=num_starts)

            real_batch_size = td.size(0) // num_starts
            device = td.device
            GREEDY_COUNT =num_starts // 2 #num_starts // 2 #num_starts // 1
            SAMPLE_COUNT = num_starts - GREEDY_COUNT

            # 2. 计算概率分布
            # 取第一个 Start 的分布作为代表 (Batch, N)
            ...
            # 2. 负数变 0 (解决 CUDA error: val < zero)
            prob_distribution = torch.relu(prob_distribution)

            # [警告] 绝对不要在这里加 +1e-8 !!!
            # 否则 mask 为 0 的非法节点会变成 1e-8，导致被选中，进而导致 -inf 报错。

            # 归一化 (防止浮点误差导致和略微大于 1 或小于 1)
            # 加上一个极小的 epsilon 仅用于分母防止除以 0 (如果整行全是 0 说明状态错了，但这步能防崩)
            prob_sum = prob_distribution.sum(dim=-1, keepdim=True)
            prob_distribution = prob_distribution / (prob_sum + 1e-16)

            # --- Part 1: 贪婪选择 ---
            topk_indices = torch.empty(real_batch_size, 0, dtype=torch.long, device=device)

            if GREEDY_COUNT > 0:
                # 确保 K_safe 不超过实际可行节点数 (过滤掉概率为 0 的)
                valid_count = (prob_distribution > 0).sum(dim=-1).min().item()
                K_safe = min(GREEDY_COUNT, prob_distribution.size(1), valid_count)
                # 如果 valid_count 为 0，说明有些 batch 无路可走，这里可能需要防御性逻辑，但通常不会发生
                if K_safe == 0: K_safe = 1 # 兜底
                ...
                if K_safe < GREEDY_COUNT:
                    ...
                    if top_idx.size(1) < GREEDY_COUNT:
                        extra = top_idx[:, :GREEDY_COUNT - top_idx.size(1)]
                        top_idx = torch.cat([top_idx, extra], dim=1)
                    top_idx = top_idx[:, :GREEDY_COUNT]

                topk_indices = top_idx

            # --- Part 2: 采样选择 ---
            selected_sample = torch.empty(real_batch_size, 0, dtype=torch.long, device=device)
