...
def rollout(model, env, eval_type='greedy'):
    ...
    t = 0
    while not done:
        cur_dist,current_node = env.get_cur_feature()
...
class Logger(object):
  ...
  def log(self, info):
    ...
    # 假设 self.filename 类似 'log/CVRP_-ts10-30-12-20-39'
    p = Path(self.filename)
    p.parent.mkdir(parents=True, exist_ok=True)
    with p.open('w', encoding='utf-8') as f:
        json.dump(self.logger, f, ensure_ascii=False, indent=2, default=str)
