...
class CVRPEnv:
    ...
    def load_vrplib_problem_batch(self, instance_list, max_size=200):
        ...
        self.reset_state.mask = self.visited_ninf_flag
        self.problem_size = self.reset_state.node_xy.shape[1]
        self.dist = (self.depot_node_xy[:, :, None, :] - self.depot_node_xy[:, None, :, :]).norm(p=2,
                                                                                                 dim=-1) + 1e6 * torch.eye(
    ...
    def reset(self):
        ...
        # shape: (batch, multi)
        self.selected_node_list = torch.zeros(size=(self.batch_size, self.multi_width, 0), dtype=torch.long,
                                              device=self.device)
        # shape: (batch, multi, 0~)
        ...
        # shape: (batch, multi)
        self.visited_ninf_flag = torch.zeros(size=(self.batch_size, self.multi_width, self.problem_size + 1),
                                             device=self.device)
        # shape: (batch, multi, problem+1)
        ...
        if self.input_mask is not None:
            self.visited_ninf_flag = self.input_mask[:, None, :].expand(self.batch_size, self.multi_width,
                                                                        self.problem_size + 1).clone()

        self.ninf_mask = torch.zeros(size=(self.batch_size, self.multi_width, self.problem_size + 1),
                                     device=self.device)
        # shape: (batch, multi, problem+1)
    ...
    def step(self, selected):
        ...
        self.depot_flag = torch.cat((self.depot_flag, (self.at_the_depot.long() - self.finished.long())[:, :, None]),
                                    dim=2)
    ...
    def get_cur_feature(self):
        ...
        current_node = self.current_node[:, :, None, None].expand(self.batch_size, self.multi_width, 1,
                                                                  self.problem_size + 1)

        # Compute the relative distance
        cur_dist = torch.take_along_dim(
            self.dist[:, None, :, :].expand(self.batch_size, self.multi_width, self.problem_size + 1,
                                            self.problem_size + 1),
            current_node, dim=2).squeeze(2)
        # shape: (batch, multi, problem)
        return cur_dist, current_node
