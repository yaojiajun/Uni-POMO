...
class VRPModel(nn.Module):
    ...
    def forward(self, state):
        ...
        # ==========================================================
        # Step 1: Return to Depot (First Move)
        # ==========================================================
        ...
        # ==========================================================
        # Step 2: Initial Node Selection (Second Move, POMO Start)
        # ==========================================================
            ...
            # 2. Obtain Current Probability Distribution
            # Note: Ensure parameters (like state.time) match Decoder definition
            probs = self.decoder(encoded_last_node, state.load, cur_dist, ninf_mask=state.ninf_mask)

            # --- Core Logic: Dynamic Expansion Based on Problem Scale (Enhanced) ---
            # Get problem size N (total nodes - 1 Depot)
            problem_size = state.ninf_mask.size(-1) - 1
            a = 1 / (1 + 2 ** ((100 - problem_size) / 50))
            base_greedy_count = int(a*100)
            base_sample_count = problem_size - base_greedy_count

            # [Key Modification] Calculate repetition factor, ensuring a minimum of 1
            # Prevents empty intermediate variables even when N=1000, pomo=100
            aug = max(1, pomo_size // problem_size)

            # Extract distribution baseline
            prob_distribution = probs[:, 0, :]  # shape: (Batch, Problem+1)

            # Numerical smoothing
            epsilon = 1e-10
            mask_active = (state.ninf_mask[:, 0, :] == 0).float()
            prob_distribution = (prob_distribution + epsilon) * mask_active
            prob_distribution = prob_distribution / prob_distribution.sum(dim=1, keepdim=True)

            # --- A. Basic Greedy Portion ---
            ...
            # --- C. Dimension Correction (For cases where pomo_size < problem_size) ---
            # Truncate if generated trajectories exceed required pomo_size
            if selected.size(1) > pomo_size:
                selected = selected[:, :pomo_size]
                prob = prob[:, :pomo_size]

            # --- D. Remainder Padding (For cases where pomo_size > generated trajectories) ---
        ...
        # ==========================================================
        # Step 3 and Beyond: Regular Decoding (Subsequent Moves)
        # ==========================================================
        else:
            batch_idx = torch.arange(batch_size).unsqueeze(1).expand(-1, pomo_size)
            selected_coords = self.all_nodes_xy[batch_idx, state.current_node]
            ...
            if self.training or self.model_params['eval_type'] == 'softmax':
                while True:  # Fix multinomial bug when probability is 0
                    with torch.no_grad():
...
class VRP_Encoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_demand):
        ...
        # 3 features: x_coord, y_coord, demands
...
class VRP_Decoder(nn.Module):
    ...
    def set_q2(self, encoded_q2):
        # encoded_q2.shape: (batch, n, embedding)
        head_num = self.model_params['head_num']
    ...
    def forward(self, encoded_last_node, load, cur_dist, ninf_mask):
        ...
        # ============================================================
        # [Core Modification] Adaptive Balanced Gating Injection
        # ============================================================
        ...
        # B. Prepare Signals
        # Physical signal: Closer distance -> smaller norm_dist -> larger -log (Reward)
        raw_heuristic = -torch.log(norm_dist + 1e-6)

        # Noise signal (Ablation: can switch between Uniform/Gaussian/Exponential)
        raw_noise = torch.rand_like(score_scaled)

        N = cur_dist.size(-1)
        # C. Calculate Complementary Weights (a + b = 1)
        # Use sigmoid to ensure 'a' stays in [0, 1] interval
        a = 1 / (1 + 2 ** ((100 - N) / 50))
        b = 1.0 - a
